...
<template>
    <li class="flex items-center ">
        ...
        <button :id="lowerName"
            ...
            <div :id="`${lowerName}-children`" class="z-10 hidden font-normal bg-white divide-y divide-gray-100 rounded-lg shadow w-44 dark:bg-gray-700 dark:divide-gray-600">
                <ul class="py-2 text-sm text-gray-700 dark:text-gray-400" :aria-labelledby="lowerName">
                  <li v-for="({name, path}) in children"
                      :key="name">
                      <RouterLink :key="name"
                                  :to="path"
